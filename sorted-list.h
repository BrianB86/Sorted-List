...
/*
 * Node type. Holds a void pointer, a reference counter, and a pointer to the next node
 */
...
/*
 * Creates a new node, returns pointer to node if success, else returns null.
 */
node *NodeCreate(void* obj,node* next);

/*
 * Destroys a node and frees it's memory.
 * Called by SLNextItem when refCnt == 1
 * Also deduct's 1 from reference counter of the next node if next != NULL
 */
